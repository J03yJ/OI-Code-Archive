...
每个触点最基本的只有向上或者向下两种模式，对于一对方向不同的触点向左绕和向右绕没有什么本质区别，因此直接钦定向左绕。

考虑触点 $(l_1,r_1),(l_2,r_2)$ 之间的关系，不失一般性地认为 $l_1<l_2$：

-   若 $r_1<l_2$，则无限制。
-   若 $l_2<r_1<r_2$，简单地枚举情况发现触点 $l_2$ 和 $r_1$ 的状态必然不同。
-   若 $r_1>r_2$，那么触点 $l_2$ 和 $r_2$ 的状态一定相同。

因此可以根据这些限制进行二分图染色，找出一组特解输出即可，时间复杂度 $O(n^2)$。
...
如果是最大则可以超级钢琴，最小不是很好刻画，因此考虑直接对 DP 状态进行图论建模，跑 $k$ 短路。

$k$ 短路即：

-   找出原有向图最短路树，则一组满足某项起点是上一项终点非树边序列与 $S\rightsquigarrow T$ 的一条路径构成双射。
-   那么维护每个节点作为起点的非树边集合，利用可持久化可并堆下放到子树内的节点，然后从 $S$ 开始超级钢琴。
...
由于要求边数最少，因此删边和把边权变为 $0$ 等价。而图上没有奇环等价于非树边构成的环没有奇环，因此考虑给每个环随机赋权，则目标为找出一个边集使得异或和恰为所有奇环的权的异或和，这是可以用 map 简单实现的。时间复杂度 $O(n\log n)$。
...
设 $f_{i,j}$ 表示当前考虑到第 $i$ 位，漏了 $j$ 个音符且 $i$ 没敲的最大收益，有 2D/1D 转移：$\displaystyle f_{i,j}=\max_{k=1}^{i-1} \big(f_{k,j-1}+w(k,i)\big)$，其中 $\displaystyle w(k,i)=\sum_{d=k+1}^{i-1}A_dC_{d-k}+P[k\neq i-1]$。

不难发现，$k\lt i-1$ 的 $w(k,i)$ 满足四边形不等式，可以决策单调性优化，$k=i-1$ 单独转移即可，时间复杂度 $O(n^2\log n)$。
...
首先只有一个点就是只有 $(n,n)$ 的情况黑方胜出。

通过手玩大量数据，发现事实上 $x=y$ 左边和右边分别只会有一个有效的点，因为这两边分别限制了白棋向上/右逃逸，因此同一方向没必要用两个棋子限制。故不妨先考虑只有两个棋子的情况。

考虑从黑棋向其右下画出其控制线，即白棋若碰到控制线则可以转化到 $(n,n)$ 的情况。并且贪心地，$x<y$ 的黑棋（称为 $(x_1,y_1)$）只会向下移动，$x>y$ 的黑棋（称为 $(x_2,y_2)$）只会向左移动。特别地，暂且先认为 $x_1\leq x_2,y_2\leq y_1$。

根据控制线的轨迹，若白棋处于 $x\leq x_1,y\leq y_2$ 的部分则仍未脱离控制，白棋为了脱离控制一定会向 $(x_1+1,y_2+1)$ 移动。退两步说，当白棋位于 $x+y=x_1+y_2$ 时，黑棋的理想位置是 $(x_1,y_2+2)$ 和 $(x_1+2,y_2)$ 或更深。因为 $(x_1,y_2)$ 被直接控制，而剩下的点则位于控制线之外，若是退一步则根据黑棋的位置以及白棋方向的选择有分讨以及不便。因此在白棋抵达 $(x_1,y_2)$ 之前黑棋需达到理想位置，即：$\max(y_1-y_2-2,0)+\max(x_2-x_1-2,0)\leq x_1+y_2$ 则黑方胜出。

$x_1\gt x_2$ 的情况可以发现左上直线的上半部分是不会用于控制白棋的，因此 $(x_1,y_1)$ 可以等效成 $(x_2,y_1-x_1+x_2)$，通过类似的分析可以规约到上面的式子。

因此仅需判断是否存在这样一对棋子即可，对 max 的取值进行分类讨论，可以在 $O(n\log n)$ 是时间内解决。
...
$k=1$ 时就是最小割，$k\gt 1$ 时考虑分层建立最小割模型，理论复杂度 $O(nm^2k^3)$，实际跑的飞快。
...
首先想到 SG 函数，朴素转移是 2D/1D 的。

由于每个点至多只有 3 叉，因此 SG 函数的值域是 $[0,3]$，考虑暴力维护前缀后缀函数复合，转移变为 2D/0D。
...
考虑抽出 DFS 生成树，尽量每条到根链只用 $7$ 种颜色染色，然后让非树边成为第 $8$ 种颜色。

设 $f_{x,k}$ 表示 $x$ 到根链上共有 $k$ 种颜色，是否可以染色使得子树内每条到根链只用 $7$ 种颜色染色，边界是 $f_{x,8}=0$。

有转移：$\displaystyle f_{x,k}=[\sum_{y\in S_x} 1-f_{x,k+1}\leq k-1]$，意义即为尽可能地让能承受多一条新颜色边的增加新颜色。

令 $F(k)$ 表示使 $f_{x,k}=0$ 最小的 $siz_x$，有转移：$F(k)=nF(k+1)+1,F(8)=1$，因此 $\displaystyle F(1)=\sum_{i=1}^7i!\gt 5555$，因此一定可以按这种方式构造出合法解，输出方案是简单的。

时间复杂度分析出来应该是 $O(n\log n)$ 的。
...
那么假设当前有 $c$ 个一样的数，调整法可知，**基本上**除了填成 $c$ 个长度为 $1$ 的谷段或填 $1$ 个长度为 $c$ 的段以外均不优。特别地，填成 $c$ 个长度为 $1$ 的谷段有 $1$ 的额外贡献。不从上述式子刻画这点则没那么显然。
