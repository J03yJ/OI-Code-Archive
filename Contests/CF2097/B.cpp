...
int px[M],py[M],ban[M],vis[M],fa[M],siz[M][2],esiz[M],f[M][2],n,m,k;
...
inline void Clear(){
    for(int i=1;i<=n*m;i++) ban[i]=fa[i]=siz[i][0]=siz[i][1]=f[i][0]=f[i][1]=vis[i]=fi[i]=esiz[i]=0;
    adj=0;
}
inline void Solve(){
    cin>>n>>m>>k;
    ...
    for(int i=1;i<=n*m;i++){
        ...
        DFS(i);
        esiz[i]>>=1;
        if(siz[i][1]>siz[i][0]) ans=0;
        if(siz[i][0]+siz[i][1]==esiz[i]) ans=2ll*ans%mod;
        else if(siz[i][0]+siz[i][1]==esiz[i]+1) ans=1ll*f[i][1]*ans%mod;
    }
    cout<<ans<<endl;
    Clear();
}
