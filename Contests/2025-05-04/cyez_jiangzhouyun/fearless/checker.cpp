...
void divide(vector <int> &x,int dep){
    ...
    fr1(i,1,(int)lef.size()-1){
        if(a[lef[i]]==a[lef[i-1]]) cout<<"L"<<a[lef[i]]<<' '<<a[lef[i-1]]<<' '<<lef[i]<<' '<<lef[i-1]<<endl,quitf(_wa,"Jeremy made a mind jam."); 
    }
    fr1(i,1,(int)rig.size()-1){
        if(a[rig[i]]==a[rig[i-1]]) cout<<"R"<<a[rig[i]]<<' '<<a[rig[i-1]]<<' '<<rig[i]<<' '<<rig[i-1]<<endl,quitf(_wa,"Jeremy made a mind jam."); 
    }
    divide(lef,dep-1);
    divide(rig,dep-1);
}
...
int main(int argc, char* argv[]){
    ...
    tc=inf.readInt(1,30);
    for(int i=1;i<=tc;i++) cout<<"# "<<i<<endl,solve();
