...
bool CalcBool(string s){
    EraseSpecialChar(s);
    int pos1=s.find(",");
    s[pos1]='#';
    int pos2=s.find(",");
    int x=CalcVal(s.substr(pos1+1,pos2-pos1-1));
    int y=CalcVal(s.substr(pos2+1));
    string op=s.substr(0,pos1);
    EraseSpecialChar(op);
    if(op=="eq") return x==y;
    else if(op=="neq") return x!=y;
    else if(op=="lt") return x<y;
    else if(op=="gt") return x>y;
    else if(op=="le") return x<=y;
    else if(op=="ge") return x>=y;
}
    ...
    for(int cur=0;cur<opt.size();){
        if(opt[cur]=="vars"){
            ...
            *FindAdd(add)=CalcVal(val);
            ...
            cout<<CalcVal(tmp)<<' ';
            cur++;
        }else if(opt[cur].find("ihu")!=string::npos){
            string tmp=opt[cur];
            tmp.erase(0,3);
            if(CalcBool(tmp)) cur++;
            else cur=nxt[cur]+1;
