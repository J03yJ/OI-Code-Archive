...
<<<<<<< HEAD
	...
	Init();
	for(int i=1;i<=n;i++) /* cerr<<i<<' '<<st[i]<<' '<<ed[i]<<endl, */Insert(1,1,q,st[i],ed[i],eu[i],ev[i]);
...
<<<<<<< HEAD
			...
			// cout<<" -> "<<ans<<' '<<Query(1,1,mnr-1,mnl-1)[0]-1ll*(mnr-1)*(mnl-1)<<endl;
			ans+=Query(1,1,mnr-1,mnl-1)[0]-1ll*(mnr-1)*(mnl-1);
			// cout<<" -> "<<ans<<' '<<Query(1,1,mxr,mxl)[0]-1ll*mxr*mxl<<endl;
			ans+=Query(1,1,mxr,mxl)[0]-1ll*mxr*mxl;
			// cout<<" -> "<<ans<<' '<<Query(1,1,mnr-1,mxl)[0]-1ll*(mnr-1)*mxl<<endl;
